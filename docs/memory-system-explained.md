...
摘要一旦提交，就会按“归档覆盖区间”确定性进入下一轮请求，而不是靠问题关键词碰运气：

```text
已提交归档覆盖到消息序号 A
承接摘要：覆盖到 A 的全部 Episode，按来源顺序渲染，必带
近期原文：A 之后、当前用户消息之前的完整 Turn，必带
当前输入：当前用户消息，只出现一次
长期记忆等其他资料：按相关度选择，可选
```

因此，即使用户下一句与旧话题毫无关键词重叠，摘要也会随请求发送；某个渲染后超过旧召回预算的大摘要同样不会被整条跳过。系统不会为了塞进预算而悄悄丢掉摘要或最近几轮：如果确实放不下，会先尝试把更早的完整前缀归档，仍放不下则返回明确的容量错误。

默认保护最近 5 个完整历史 Turn（有模型配置时使用 `history_min_turns`）。已提交摘要的边界会与正文、版本、预算一起固定；用户编辑或删除摘要后，旧副本不会被重新注入。
