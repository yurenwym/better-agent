...
function ProgramDayPlan({program,localDate,busyId,onComplete}:{program:GoalProgram;localDate?:string;busyId:string|null;onComplete:(action:GoalAction)=>void}){const days=planDays(program);if(!days.length)return null;return <section className="program-day-plan" aria-label="完整执行日程"><header><div><span className="eyebrow">完整日程</span><h4>每天要做什么</h4><p>勾选待完成事项即可更新进度；计划调整后会同步更新。</p></div><strong>{days.length} 天</strong></header><div className="program-day-list">{days.map(day=><details key={day.scheduledDate} open={day.scheduledDate===localDate}><summary><span><strong>第 {day.dayNumber} 天</strong><small>{compactDate(day.scheduledDate)}</small></span><span>{day.actions.length} 项 · {day.actions.reduce((total,action)=>total+action.estimated_minutes,0)} 分钟</span></summary><ol>{day.actions.map(action=>{const label=actionStatusLabel[action.status]??"状态已更新";const canComplete=program.status==="ACTIVE"&&action.status==="SCHEDULED"&&Boolean(action.liveAction);return <li aria-busy={busyId===action.liveAction?.id} className={`plan-action-${String(action.status).toLowerCase()}`} key={action.logical_key}><label className="program-action-toggle"><input aria-label={canComplete?`标记完成：${action.title}`:`${label}：${action.title}`} checked={action.status==="COMPLETED"} className="program-action-check" disabled={!canComplete||busyId!==null} type="checkbox" onChange={()=>{if(canComplete&&action.liveAction)onComplete(action.liveAction)}}/></label><div><strong>{action.title}</strong><small>{action.required?"必做":"可选"} · {action.estimated_minutes} 分钟</small></div><em>{busyId===action.liveAction?.id?"更新中…":label}</em></li>})}</ol></details>)}</div></section>}
...
export default function TodayPage({ csrfToken, onHelp }: Props) {
  ...
  return <div className="today-page">
    <aside className="today-list" aria-label="执行项目">
      <header className="today-list-head"><div><span className="eyebrow">目标执行</span><h2>执行项目</h2><p>{new Intl.DateTimeFormat("zh-CN",{dateStyle:"long"}).format(new Date())}</p></div><span className="today-count">{programs.length}</span></header>
    ...
    <section className="today-detail" aria-label="行动详情">{selectedProgram?<section className="today-program" key={selectedProgram.id}>
      ...
      <section className="today-focus-card" aria-label="当前执行重点">
        <div><span className="eyebrow">当前重点</span><h4>{nextAction?`下一步：${nextAction.title}`:selectedProgram.status==="ACTIVE"?"今天的行动已处理完":"当前没有待执行行动"}</h4><p>{nextAction?`${compactDate(nextAction.scheduled_date)} · 预计 ${nextAction.estimated_minutes} 分钟`:"可以查看复盘和完整日程，确认接下来的节奏。"}</p></div>
      ...
      {selectedProgram.completion_summary&&<section className="goal-completion-summary"><span className="eyebrow">目标总结</span><h4>目标周期总结</h4><p>{selectedProgram.completion_summary}</p><small>已保存到长期记忆</small></section>}
