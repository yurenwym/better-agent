...
export default function TodayPage({ csrfToken, onHelp }: Props) {
  ...
  const load=useCallback(async()=>{try{setData(await getToday());}catch(reason){setError(message(reason));}},[]);
  ...
  if(!data)return <section className="today-page" aria-busy="true"><p role="status">正在加载今天的行动…</p></section>;
  return <div className="today-page page-stack">
    <section className="today-hero"><div><span className="eyebrow">TODAY / DAILY COMPANION</span><h2>今天的行动</h2><p>只推进已确认的执行安排；原计划文档仍可独立编辑。</p></div><span className="today-date">{new Intl.DateTimeFormat("zh-CN",{dateStyle:"long"}).format(new Date())}</span></section>
    {error&&<p className="error-message" role="alert">{error}</p>}
    {data.programs.length===0?<section className="empty-panel"><h3>今天没有待推进的行动</h3><p>暂停的执行项目和已结束行动不会出现在这里。</p></section>:data.programs.map(group=><section className="today-program card" key={group.program.id}>
