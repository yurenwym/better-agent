...
function rawPresentation(content: string, parsed: Record<string, unknown>): MessagePresentation {
  ...
  return { summary: "模型返回了一条结构化结果。", detail: null, bullets: [], raw: content };
}

export function presentMessage(message: MessageRecord): MessagePresentation {
  if (message.role === "user") {
    return { summary: message.content, detail: null, bullets: [], raw: null };
  }
  const parsed = parseJson(message.content);
  return parsed ? rawPresentation(message.content, parsed) : { summary: message.content, detail: null, bullets: [], raw: null };
}
