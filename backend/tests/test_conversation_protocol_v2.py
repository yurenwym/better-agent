...
@pytest.mark.asyncio
async def test_plan_shaped_answer_is_repaired_into_a_saved_document() -> None:
    import json
    from types import SimpleNamespace
    ...
    class Gateway:
        ...
        async def complete(self, request, **kwargs):
            self.requests.append(request)
            if len(self.requests) == 1:
                message = '{"v":1,"policy":"answer","content_shape":"travel_plan","reason_code":"content_only"}\n# Inner Mongolia\n'
            elif len(self.requests) == 2:
                message = '{"plan_document_request":true}'
            else:
                message = (
                    '{"v":2,"policy":"answer","content_shape":"plan_document",'
                    '"reason_code":"plan_document_upsert","artifact":{"kind":"plan_document",'
                    '"operation":"upsert","title":"Inner Mongolia plan"}}\n# Inner Mongolia\n'
                )
            if len(self.requests) != 2:
                kwargs["on_text_delta"](message)
    ...
    assert json.loads(result.message.splitlines()[0])["artifact"]["operation"] == "upsert"
    assert len(gateway.requests) == 3
    assert gateway.requests[1].tools == []
    assert gateway.requests[2].tools == []
...
@pytest.mark.asyncio
async def test_classifier_failure_preserves_a_valid_ask_request() -> None:
    import json
    from types import SimpleNamespace
    ...
    class Gateway:
        ...
        async def complete(self, request, **kwargs):
            ...
            raise GatewayError("classifier unavailable", "server")

    result = await LiveConversationModel(Gateway()).route_and_respond(
        content="I want a personalized training plan",
        history=[],
        skill_names=[],
        on_text_delta=lambda _: None,
        on_text_reset=lambda: None,
    ...
    assert result.questions[0].id == "context"
