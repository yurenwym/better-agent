...
def research_write_fragment(policy: Any) -> str:
    """Return only the release-approved researcher/write fragment.

    Legacy unstructured policies remain readable for old bundles, while M5
    structured policies cannot leak into plan/distill/audit prompts.
    """
    if not isinstance(policy, dict):
        return DEFAULT_EVIDENCE_STATEMENT
    value: Any = policy
    for key in ("researcher", "write_research_section", "evidence_statement"):
        if not isinstance(value, dict) or key not in value:
            return DEFAULT_EVIDENCE_STATEMENT
        value = value[key]
    return value.strip() if isinstance(value, str) and value.strip() else DEFAULT_EVIDENCE_STATEMENT


def build_research_write_messages(policy: Any, heading: str, thesis: str, evidence: Any, prior_summary: str) -> list[dict[str, str]]:
    fragment = research_write_fragment(policy)
    system = UNTRUSTED + "只撰写所请求的简洁 Markdown 章节。" + fragment + "不要输出标题。"
    # A legacy policy was global by design. Preserve old bundle replay, but a
    # structured M5 policy contributes only the single allowed leaf above.
    if policy not in (None, "", "live-model-v1") and not isinstance(policy, dict):
        system = "应用以下已经批准的 Better Agent 运行时提示词策略：\n" + json.dumps(policy, ensure_ascii=False) + "\n\n" + system
    return [
...
class LiveResearchModel:
    ...
    def _system(self, instruction: str) -> str:
        ...
        if isinstance(policy, dict) and "researcher" in policy:
            return instruction
        return "应用以下已经批准的 Better Agent 运行时提示词策略：\n" + json.dumps(policy, ensure_ascii=False) + "\n\n" + instruction

    def render_write_messages(self, heading, thesis, evidence, prior_summary) -> list[dict[str, str]]:
        policy = self.runtime_prompt_policy() if self.runtime_prompt_policy is not None else None
    ...
    async def distill(self, source, topic: str, sections: tuple[str, ...]):
        ...
        for item in data.get("evidence", [])[:6]:
            ...
            if not text or " ".join(text.split()) not in " ".join(excerpt.split()):
                continue
    ...
    def fallback_distill(source, topic: str, sections: tuple[str, ...]):
        ...
        sentences = [
            ...
            and re.search(r"[。！？.!?:：;]$", item.strip())
            and not re.search(r"Copyright|Supported Versions|Released!|Documentation:", item, re.I)
        ]
        selected = []
        intents = [*sections, *source.metadata.get("queries", ()), str(source.metadata.get("query", "")), topic]
        for intent in intents:
            intent = re.sub(r"https?://\S+|site:\S+|\[ACCEPT[^\]]*\]", "", intent, flags=re.I)
            terms = {item.lower() for item in re.findall(r"[a-zA-Z0-9][a-zA-Z0-9_.+#-]{1,}|[\u4e00-\u9fff]{2,}", intent)}
            terms -= {"the", "and", "for", "with", "postgresql", "docs", "current"}
