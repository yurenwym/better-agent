...
from .live_model import LiveRuntimeModel
...
def build_runtime(data_root: str | Path, profile: ModelProfile | None = None, llm_ap_path: str | Path | None = None) -> AgentRuntime:
    ...
    model = LiveRuntimeModel(ModelGateway(configured_profile), tools.describe()) if configured_profile else MockModelGateway()
    return AgentRuntime(
        db=db,
        events=events,
